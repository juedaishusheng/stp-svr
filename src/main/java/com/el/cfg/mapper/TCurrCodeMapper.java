...
import com.el.cfg.domain.TCurrCode;
...
public interface TCurrCodeMapper {
    @Delete({
        "delete from T_CURR_CODE",
        "where ID = #{id,jdbcType=DECIMAL}"
    })
    int deleteByPrimaryKey(Long id);

    @Insert({
        "insert into T_CURR_CODE (ID, OU_ID, ",
        "MCU_ID, CURR_CODE, CURR_NAME, ",
        "CURR_DISP_DECIMAL, LOCKER_ID, ",
        "LOCK_TS, MODIFYER_ID, ",
        "MODIFY_TS, MODIFY_APPSVR, ",
        "MODIFY_MODULE, DATA_VERSION)",
        "values (#{id,jdbcType=DECIMAL}, #{ouId,jdbcType=DECIMAL}, ",
        "#{mcuId,jdbcType=DECIMAL}, #{currCode,jdbcType=NCHAR}, #{currName,jdbcType=NCHAR}, ",
        "#{currDispDecimal,jdbcType=DECIMAL}, #{lockerId,jdbcType=DECIMAL}, ",
    ...
    @SelectKey(statement="SELECT SEQ_T_CURR_CODE.NEXTVAL FROM DUAL", keyProperty="id", before=true, resultType=Long.class)
    int insert(TCurrCode record);

    @Select({
        "select",
        "ID, OU_ID, MCU_ID, CURR_CODE, CURR_NAME, CURR_DISP_DECIMAL, LOCKER_ID, LOCK_TS, ",
    ...
    @Results({
        ...
        @Result(column="CURR_NAME", property="currName", jdbcType=JdbcType.NCHAR),
        @Result(column="CURR_DISP_DECIMAL", property="currDispDecimal", jdbcType=JdbcType.DECIMAL),
    ...
    TCurrCode selectByPrimaryKey(Long id);
    ...
    @Update({
        ...
        "set OU_ID = #{ouId,jdbcType=DECIMAL},",
          ...
          "CURR_NAME = #{currName,jdbcType=NCHAR},",
          "CURR_DISP_DECIMAL = #{currDispDecimal,jdbcType=DECIMAL},",
    ...
    int updateByPrimaryKey(TCurrCode record);
}