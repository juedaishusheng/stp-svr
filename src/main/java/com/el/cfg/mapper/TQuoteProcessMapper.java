...
import com.el.cfg.domain.TQuoteProcess;
...
public interface TQuoteProcessMapper {
    @Delete({
        "delete from T_QUOTE_PROCESS",
        "where ID = #{id,jdbcType=DECIMAL}"
    })
    int deleteByPrimaryKey(Long id);

    @Insert({
        "insert into T_QUOTE_PROCESS (ID, QUOTE_ID, ",
        "LINE_NO, ROUTE_ID, ",
        "PROCESS_ID, PROD_ID, ",
        "PROD_PART_ID, ITEM_QTY, ",
        "ITEM_UOM, TRANS_TS, ",
        "REMARK, HOME_CURR_CODE, ",
        ...
        "#{itemUom,jdbcType=NCHAR}, #{transTs,jdbcType=TIMESTAMP}, ",
        "#{remark,jdbcType=NCHAR}, #{homeCurrCode,jdbcType=NCHAR}, ",
    ...
    @SelectKey(statement="SELECT SEQ_T_QUOTE_PROCESS.NEXTVAL FROM DUAL", keyProperty="id", before=true, resultType=Long.class)
    int insert(TQuoteProcess record);

    @Select({
        "select",
        "ID, QUOTE_ID, LINE_NO, ROUTE_ID, PROCESS_ID, PROD_ID, PROD_PART_ID, ITEM_QTY, ",
        "ITEM_UOM, TRANS_TS, REMARK, HOME_CURR_CODE, FORE_CURR_CODE, CURR_RATE, CALC_PRICE, ",
        "CURR_CALC_PRICE, QUOTE_PRICE, CURR_QUOTE_PRICE, CALC_AMT, CURR_CALC_AMT, QUOTE_AMT, ",
        "CURR_QUOTE_AMT, PRICE_TYPE, PRICE_CONFIRM_DESC, LOCKER_ID, LOCK_TS, MODIFYER_ID, ",
        "MODIFY_TS, MODIFY_APPSVR, MODIFY_MODULE, DATA_VERSION",
        "from T_QUOTE_PROCESS",
        "where ID = #{id,jdbcType=DECIMAL}"
    })
    @Results({
    ...
    TQuoteProcess selectByPrimaryKey(Long id);
    ...
    List<TQuoteProcess> selectAll();

    @Update({
        "update T_QUOTE_PROCESS",
    ...
    int updateByPrimaryKey(TQuoteProcess record);
}