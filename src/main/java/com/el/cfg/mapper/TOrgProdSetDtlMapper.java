...
import com.el.cfg.domain.TOrgProdSetDtl;
...
public interface TOrgProdSetDtlMapper {
    @Delete({
        "delete from T_ORG_PROD_SET_DTL",
        "where ID = #{id,jdbcType=DECIMAL}"
    })
    int deleteByPrimaryKey(Long id);

    @Insert({
        "insert into T_ORG_PROD_SET_DTL (ID, SET_ID, ",
        "LINE_NO, PROD_ID, ",
    ...
    @SelectKey(statement="SELECT SEQ_T_ORG_PROD_SET_DTL.NEXTVAL FROM DUAL", keyProperty="id", before=true, resultType=Long.class)
    int insert(TOrgProdSetDtl record);
    ...
    TOrgProdSetDtl selectByPrimaryKey(Long id);
    ...
    List<TOrgProdSetDtl> selectAll();

    @Update({
        "update T_ORG_PROD_SET_DTL",
    ...
    int updateByPrimaryKey(TOrgProdSetDtl record);
}