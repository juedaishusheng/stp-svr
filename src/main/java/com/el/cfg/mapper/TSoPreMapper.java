...
import com.el.cfg.domain.TSoPre;
...
public interface TSoPreMapper {
    @Delete({
        "delete from T_SO_PRE",
        "where ID = #{id,jdbcType=DECIMAL}"
    })
    int deleteByPrimaryKey(Long id);

    @Insert({
        "insert into T_SO_PRE (ID, T_SO_PROD_ID, ",
    ...
    @SelectKey(statement="SELECT SEQ_T_SO_PRE.NEXTVAL FROM DUAL", keyProperty="id", before=true, resultType=Long.class)
    int insert(TSoPre record);
    ...
    TSoPre selectByPrimaryKey(Long id);
    ...
    int updateByPrimaryKey(TSoPre record);
}