...
import com.el.cfg.domain.TSoPaper;
...
public interface TSoPaperMapper {
    @Delete({
        "delete from T_SO_PAPER",
        "where ID = #{id,jdbcType=DECIMAL}"
    })
    int deleteByPrimaryKey(Long id);

    @Insert({
        "insert into T_SO_PAPER (ID, LINE_NO, ",
        "PROD_PART_ID, PAPER_TYPE, ",
        "PAPER_ITEM_ID, ITEM_UOM, ",
        ...
        "ROUTE_ID, PROCESS_ID, ",
        "PROD_ID, ITEM_QTY)",
        "values (#{id,jdbcType=DECIMAL}, #{lineNo,jdbcType=DECIMAL}, ",
        "#{prodPartId,jdbcType=DECIMAL}, #{paperType,jdbcType=DECIMAL}, ",
        "#{paperItemId,jdbcType=DECIMAL}, #{itemUom,jdbcType=NCHAR}, ",
        ...
        "#{dataVersion,jdbcType=DECIMAL}, #{soId,jdbcType=DECIMAL}, ",
        "#{routeId,jdbcType=DECIMAL}, #{processId,jdbcType=DECIMAL}, ",
        "#{prodId,jdbcType=DECIMAL}, #{itemQty,jdbcType=DECIMAL})"
    })
    @SelectKey(statement="SELECT SEQ_T_SO_PAPER.NEXTVAL FROM DUAL", keyProperty="id", before=true, resultType=Long.class)
    int insert(TSoPaper record);

    @Select({
        "select",
        "ID, LINE_NO, PROD_PART_ID, PAPER_TYPE, PAPER_ITEM_ID, ITEM_UOM, HOME_CURR_CODE, ",
        "FORE_CURR_CODE, CURR_RATE, CALC_PRICE, CURR_CALC_PRICE, QUOTE_PRICE, CURR_QUOTE_PRICE, ",
        "CALC_AMT, CURR_CALC_AMT, QUOTE_AMT, CURR_QUOTE_AMT, PRICE_TYPE, PRICE_CONFIRM_DESC, ",
        "REMARK, LOCKER_ID, LOCK_TS, MODIFYER_ID, MODIFY_TS, MODIFY_APPSVR, MODIFY_MODULE, ",
        "DATA_VERSION, SO_ID, ROUTE_ID, PROCESS_ID, PROD_ID, ITEM_QTY",
        "from T_SO_PAPER",
        "where ID = #{id,jdbcType=DECIMAL}"
    })
    @Results({
    ...
    TSoPaper selectByPrimaryKey(Long id);
    ...
    List<TSoPaper> selectAll();

    @Update({
        "update T_SO_PAPER",
        "set LINE_NO = #{lineNo,jdbcType=DECIMAL},",
    ...
    int updateByPrimaryKey(TSoPaper record);
}