...
define(function (require) {
  ...
  var listView_reqProxy = {
    url: 'api/pm/pm004/listView',
    argsBuilder: argsBuilder,
    callback: renderTable,   //回调
    sortSelector: '.pm004>.listView .table-master',
    pagingSelector: '.pm004>.listView .stp-pm004-list',
  ...
  function argsBuilder() {
    var custName = $('.pm004 .listView .custName').val();
    var prodName = $('.pm004 .listView .prodName').val();
    var quoteNo = $('.pm004 .listView .quoteNo').val();
  ...
  //主页面Table数据点击跳转到子页面事件处理
  function goToDetail() {
    $('.pm004 .listView .listView-data').on('click', 'a', function () {
      $temp = $(this).data('args');
      $tableDate = $temp.split(",");
      var id = $tableDate[0];
      var quoteNo = $tableDate[1];
      var quoteStatus = $tableDate[2];
      EL.log('!!!!!$tableDate is:'+$tableDate);
      EL.log('!!!!!id||quoteNo||quoteStatusis:'+id+"||"+quoteNo+"||"+quoteStatus);
      /*view("createPriceBtn");*/
      optClass('editView');
      loadDblData();
      /*listenEditClick();*/
...
function enterPay() {
  ...
  function loadDblData() {
    document.getElementById("quoteNo").value="20160826001";
    document.getElementById("custCode").value="1002";
    document.getElementById("custName").value="测试用户001";
    document.getElementById("dueDate").value="2016-08-26";
    document.getElementById("agentName").value="业务员001";
    document.getElementById("depName").value="业务部门001";
    document.getElementById("quoteDate").value="2016-08-26";
    document.getElementById("effectDate").value="2016-08-26";
    document.getElementById("payMentMethod").value="信用卡支付001";
    document.getElementById("payMentTerms").value="先验货后付款001";
    /*return $.when(
