...
define(function (require) {
  ...
  var listView_reqProxy = {
    url: 'api/pm/pm002/listView',
    argsBuilder: argsBuilder,
    callback: renderTable,   //回调
    sortSelector: '.pm002>.listView .table-master',
    pagingSelector: '.pm002>.listView .stp-pm002-list',
  ...
  function argsBuilder() {
    var checkboxObj = $('.pm002 .listView .prodTypeCheckbox input[name="pm002-checkbox"]:checked');
    var check_array = new Array();
    checkboxObj.each(function () {
      check_array.push($(this).val());
    });

    var custName = $('.pm002 .listView .custName').val();
    var prodName = $('.pm002 .listView .prodName').val();
    var quoteNo = $('.pm002 .listView .quoteNo').val();
...
function view(viewId) {
  if (viewId === 'listView') {
    ...
    dlg.find('.modal-body').html($('.pm002 .payDetail').html());
