...
define(function (require) {
  ...
  var delinoteProdView_reqProxy = {
    url: 'api/fm/fm002/delinoteProdView',
    argsBuilder: argsBuilder,
  ...
  function loadPayTotalData(){
      ...
      $.when(
        EL.get('api/fm/fm002/payTotal',param)
      ).then(function (total) {
  ...
  function loadListTotalData(){
    var param = argsBuilder();
    $.when(
      EL.get('api/fm/fm002/listTotal',param)
    ).then(function (total) {
  ...
  function loadTicketTotalData(){
    ...
    var param = argsBuilder();
    $.when(
      EL.get('api/fm/fm002/ticketTotal',param)
    ).then(function (total) {
      EL.log("11111",total);
  ...
  function loadStatementTotalData(){
    var param = argsBuilder();
    $.when(
      EL.get('api/fm/fm002/statementTotal',param)
    ).then(function (total) {
...
$('.class').val()
  ...
  function handStatement() {
    ...
    if( soProdId == void(0)){
      ...
      //调用手动结算服务
      EL.get("api/fm/fm002/handStatementView",{soProdId:soProdId}).then(function (data) {
        if(data){
          EL.msg('结算单已生成');
        }else{
          EL.msg('结算单生成失败');
